...
### `DepartmentType`
* **Değerler:** `HR`, `FINANCE`, `IT`, `SOFTWARE_DEVELOPMENT`
...
### `UserRole`
* **Değerler:** `EMPLOYEE`, `DEPARTMENT_MANAGER`, `APPROVER`, `ADMIN`
* **Tasarım Gerekçesi:** Kullanıcıların sistemdeki yetki seviyelerini belirler. Bir kullanıcının aynı anda birden fazla role sahip olabilir. Herkes bir `EMPLOYEE` ve bir kişi `MANAGER` olabilir. Ayrıca `DEPARTMENT` içerisinde çalışanlar `APPROVER` olarak tanımlanabilir. Böylelikle her departman bazında özel bir enum tanımlamaya gerek kalmaz (örn: HR Specialist)  `User` sınıfında `@ElementCollection` ile esnek bir yapıda modellenmiştir.
...
### `RequestStatus`
* **Değerler:** `DRAFT`, `PENDING`, `APPROVED`, `REJECTED`, `CANCELLED`
...
* **Tasarım Gerekçesi:** Bir talebin içerisindeki her bir onay adımının (mikro) anlık durumunu tutar. Talebin genel durumu `PENDING` iken, sırası gelen adım `PENDING`, henüz sırası gelmeyen adımlar ise `WAITING` durumunda bekler.
...
### `User` Entity
* **Barındırdığı Alanlar:** `firstName`, `lastName`, `email`, `passwordHash`, `userRoles`, `department`
* **JPA İlişkileri ve Gerekçeleri:**
    ...
    * **`department` (`@ManyToOne`):** Kullanıcının bağlı olduğu departmanı gösterir (`department_id` foreign key). Bir departmanda birden fazla kullanıcı bulunabilir.

### `Department` Entity
* **Barındırdığı Alanlar:** `departmentType`, `manager`
* **JPA İlişkileri ve Gerekçeleri:**
    * **`departmentType` (`@Enumerated(EnumType.STRING)`):** Departman türünü tip güvenli şekilde tutar.
    * **`manager` (`@OneToOne`):** Departman yöneticisi olan kullanıcıyı tutar (`manager_id` foreign key). `DEPARTMENT_MANAGER` rolü soyut bir yetki tanımıdır; hangi kullanıcının hangi spesifik departmanın yöneticisi olduğu bilgisi `Department` üzerinden kurulmuştur.

### `Request` Entity
* **Barındırdığı Alanlar:** `requestType`, `status`, `title`, `description`, `requester`, `approvalSteps`
* **JPA İlişkileri ve Gerekçeleri:**
    * **`requester` (`@ManyToOne`):** Talebi oluşturan kullanıcıyı bağlar (`requester_id` foreign key).
...
### `ApprovalStep` Entity
* **Barındırdığı Alanlar:** `stepOrder`, `comment`, `actionDate`, `status`, `targetRoles`, `targetDepartment`, `request`, `approver`
* **JPA İlişkileri ve Gerekçeleri:**
    * **`status` (`@Enumerated(EnumType.STRING)`):** İlgili onay adımının anlık durumunu (`StepStatus`) tutar.
    * **`targetRoles` (`@ElementCollection` + `@CollectionTable`):** `approval_step_target_roles` adında ara tablo oluşturur. Bu adımı onaylamaya yetkili rolleri esnek şekilde (`Set<UserRole>`) tutar.
    * **`targetDepartment` (`@Enumerated(EnumType.STRING)`):** Adımın hangi departman onayına düşeceğini gösterir.
    * **`request` (`@ManyToOne`):** Adımın bağlı olduğu ana talebi temsil eder (`request_id` foreign key).
    * **`approver` (`@ManyToOne`):** Onay eylemini gerçekleştiren kullanıcıyı bağlar (`approver` foreign key).
    * `target_role` ve `target_department` ikisi beraber kullanılıcak olup ilgili departmanın ilgili rollerindeki kişiler onay ekranında `request` leri görebileceklerdir. İlk onaylayan `approver` olarak kayıt edilecektir.
